  ...
  try {
    ...
    if (type === "website") {
      ...
      const res = await fetch(zenUrl.toString(), {
        headers: { "User-Agent": "sovereign-support/1.0" }
      });
      ...
      if (!res.ok) {
        ...
        return NextResponse.json(
          { error: "Failed to fetch website content from ZenRows" },
          { status: 502 }
        );
      }
      ...
      // CSV upload is handled via a dedicated multipart endpoint.
      return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      message: "Knowledge source added"
    });
  } catch (error) {

...
import { chatWidgetSettings, conversations, knowledgeItems, messages, metadata } from "@/db/schema";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (userEmail) {
      ...
      brandName = orgMetadata[0]?.business_name;
    }
    ...
    let knowledgeContext = contextPieces.join("\n\n---\n\n");
    if (knowledgeContext.length > MAX_CONTEXT_CHARS) {
      knowledgeContext = knowledgeContext.slice(0, MAX_CONTEXT_CHARS);
    }
